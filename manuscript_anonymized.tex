...
Area under the ROC curve (AUC) is the dominant evaluation metric for binary
classifiers, but it reports only rank discrimination and omits several
properties that matter in practice: whether predicted probabilities are
accurate, how stable the score is across different data splits, and whether
prediction errors fall equally on all demographic groups. This study asks
whether these hidden gaps are detectable on a small benchmark dataset using
standard tools already available to most practitioners.

\textbf{Methods:}
Three classifiers---Logistic Regression, Random Forest, and CatBoost---were
trained on the IBM HR Analytics Employee Attrition dataset (1,470 records).
Beyond AUC, each model was evaluated for Expected Calibration Error (ECE)
before and after isotonic regression correction, bootstrap confidence
intervals on AUC (1,000 resamples), and demographic fairness metrics
(Demographic Parity Difference and Equalized Odds Difference) for gender
and age groups.

\textbf{Results:}
AUC scores ranged from 0.759 to 0.798, which appears acceptable by standard
benchmarks. However, CatBoost's pre-calibration ECE was 8.6\%. Bootstrap
95\% confidence intervals spanned 15--16 percentage points for all models,
reflecting genuine uncertainty at this dataset size. The age-group Equalized
Odds Difference reached 0.258 for CatBoost---a 26-point fairness gap
invisible to AUC.

\textbf{Conclusions:}
All three problems appeared simultaneously in the same models while AUC
looked acceptable. These gaps are not exotic failure modes; they are
detectable with standard scikit-learn tools in a few additional hours of
work. AUC alone is insufficient for evaluating classifiers intended for
